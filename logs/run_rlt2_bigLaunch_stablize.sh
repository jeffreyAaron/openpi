#!/usr/bin/env bash
# Stage 2 RLT (tape handover) stability preset:
# - deterministic actor rollout (no stochastic action sampling)
# - stronger BC anchoring
# - lower actor LR
# - lower UTD
#
# Based on run_rlt2_bigLaunch.sh, but with safer defaults for phase-1 collapse.
set -euo pipefail
...
uv run python scripts/train_rlt_stage2.py \
  ...
  --exp_name rlt2_bigLaunch_stablize \
